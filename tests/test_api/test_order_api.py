...
def _create_subscription(
    client: TestClient, plan_id: int, api_headers: dict[str, str]
) -> dict[str, object]:
    resp = client.post(
...
class TestCreateOrder:
    ...
    def test_create_rejects_non_one_time(
...
class TestListBySubscription:
    def test_returns_orders(
        ...
        resp = client.get(
            f"/api/v1/orders/by-subscription/{sub['id']}", headers=api_client_token_headers
        )
        assert resp.status_code == 200
        assert resp.json() == []
...
class TestCancelOrder:
    ...
    def test_cancel_paid_order_fails(
        self, client: TestClient, api_client_token_headers: dict[str, str]
    ) -> None:
        order = _create_one_time_order(client, api_client_token_headers)
        client.post(f"/api/v1/orders/{order['id']}/pay", headers=api_client_token_headers)
